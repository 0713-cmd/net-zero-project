...
if df_raw is None:
    st.error("🚨 'data.csv' 파일을 찾을 수 없습니다. GitHub에 파일이 올라갔는지 확인해주세요.")
    st.stop()

# 3. 데이터 매핑 (엑셀 행번호 그대로 적용)
# 연도: 2행(Index 1)에서 숫자만 추출 (2023~2050)
...
# 데이터 추출 함수
def get_vals(row_idx): # 엑셀 행번호 넣으면 됨
...
# 데이터 준비
title1 = get_title(1)
expected = get_vals(4) # 4행 예상배출량
target = get_vals(3)   # 3행 목표배출량
inv_red = get_vals(6)  # 6행 투자감축량
rec_red = get_vals(7)  # 7행 REC감축량

title2 = get_title(8)
g2_rows = range(10, 15) # 10~14행
g2_labels = [str(df_raw.iloc[r-1, 0]) for r in g2_rows]
g2_data = [get_vals(r) for r in g2_rows]

title3 = get_title(16)
g3_rows = range(17, 22) # 17~21행
...
fc1 = get_title(41) if len(df_raw) >= 41 else "" # 엑셀 41행을 내용으로
...
fig1 = go.Figure()
fig1.add_trace(go.Scatter(x=years, y=expected, name='예상 배출량', line=dict(color='#8B949E', width=2, dash='dash')))
fig1.add_trace(go.Scatter(x=years, y=target, name='목표 배출량', line=dict(color='#2BD6B4', width=4)))
fig1.add_trace(go.Bar(x=years, y=inv_red, name='투자 감축량', marker_color='#1E90FF'))
fig1.add_trace(go.Bar(x=years, y=rec_red, name='REC 감축량', marker_color='#FFD700'))
...
sel_year = st.select_slider("연도 선택", options=years, value="2030")
idx = years.index(sel_year)

c1, c2 = st.columns(2)
with c1: st.markdown(f'<div class="info-box"><div class="box-title">📌 {sel_year}년 이슈</div><div class="box-content">{box1[idx]}</div></div>', unsafe_allow_html=True)
with c2: st.markdown(f'<div class="info-box"><div class="box-title">💰 {sel_year}년 투자</div><div class="box-content">{box2[idx]}</div></div>', unsafe_allow_html=True)
